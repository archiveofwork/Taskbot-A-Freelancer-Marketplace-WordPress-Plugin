...
$proposal_page_url  = Taskbot_Profile_Menu::taskbot_profile_menu_link('proposals', $user_identity, true, 'listing',$project_id);
...
$taskbot_attr['project_title']  = !empty($product) ? $product->get_name() : '';
...
if( !empty($proposal_type) && $proposal_type === 'milestone'){
    ...
    if( !empty($milestone) ){
        ...
        if( !empty($milestone_total) && $milestone_total == $earned_balance ){
            $taskbot_attr['complete_option']         = 'yes';
        }
...
<div class="tk-project-wrapper tk-activities-<?php echo esc_attr($project_type);?>">
    <div class="tk-project-box tk-employerproject">
        ...
        <div class="tk-price">
            ...
            <div class="tk-project-detail">
                <a href="<?php echo esc_url($proposal_page_url);?>" class="tk-btn-solid-lg"><?php esc_html_e('View all proposals','taskbot');?></a>
            </div>
        </div>
    </div>
    <div class="tk-projectstatus">
        <?php if( !empty($no_of_freelancers) && $no_of_freelancers >1 && !empty($user_type) && $user_type === 'buyers' ){ ?>
            <div class="tk-projectstatus_users">
                <?php taskbot_get_template_part('dashboard/post-project/dashboard', 'hired-proposals',$taskbot_attr);?>
            </div>
        <?php } ?>
        <div class="tk-projectsstatus">
            <?php taskbot_get_template_part('dashboard/post-project/dashboard', 'project-seller-basic',$taskbot_attr);?>
            <?php 
                if( !empty($proposal_type) && $proposal_type === 'milestone') {
                    taskbot_get_template_part('dashboard/post-project/dashboard', 'project-milestone',$taskbot_attr);
                } else if( !empty($project_type) && $project_type != 'fixed' ){
                    do_action( 'taskbot_buyer_project_basic_details', $proposal_id );
                }
            ?>
            <div class="tk-projectsactivity">
                <ul class="nav nav-tabs tk-nav-tabs" id="projectActivitiesTabs" role="tablist">
                    ...
                    <?php do_action( 'taskbot_project_buyer_avtivity_tabs',$taskbot_attr );?>
                </ul>
                <div class="tab-content tk-project-type-content" id="projectActivitiesTabsContent">
                    ...
                    <?php 
                        ...
                        do_action( 'taskbot_project_buyer_avtivity_tab_content', $taskbot_attr );
                    ?>
                </div>
            </div>
        </div>

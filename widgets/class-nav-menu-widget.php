...
if (!class_exists('Taskup_Nav_Menu_Widget')) {
    class Taskup_Nav_Menu_Widget extends WP_Widget {
        ...
        public function __construct() {
            $widget_ops = array(
                'description'                 => __( 'Add a navigation menu to your sidebar.','taskbot' ),
                'customize_selective_refresh' => true,
                'show_instance_in_rest'       => true,
            );
            parent::__construct( 'taskup_nav_menu', __( 'Taskup navigation Menu','taskbot' ), $widget_ops );
        }
        ...
        public function form( $instance ) {
            ...
            <p class="nav-menu-widget-no-menus-message" <?php echo do_shortcode( $not_empty_menus_style ); ?>>
                ...
                printf( __( 'No menus have been created yet. <a href="%s">Create some</a>.','taskbot' ), esc_attr( $url ) );
                ?>
            </p>
            <div class="nav-menu-widget-form-controls" <?php echo do_shortcode( $empty_menus_style ); ?>>
                <p>
                    <label for="<?php echo do_shortcode( $this->get_field_id( 'title' )); ?>"><?php _e( 'Title:','taskbot' ); ?></label>
                ...
                <p>
                    <label for="<?php echo do_shortcode( $this->get_field_id( 'nav_menu' )); ?>"><?php _e( 'Select Menu:','taskbot' ); ?></label>
                    <select id="<?php echo do_shortcode( $this->get_field_id( 'nav_menu' )); ?>" name="<?php echo do_shortcode( $this->get_field_name( 'nav_menu' )); ?>">
                        <option value="0"><?php _e( '&mdash; Select &mdash;','taskbot' ); ?></option>
                ...
                <?php if ( $wp_customize instanceof WP_Customize_Manager ) : ?>
                    <p class="edit-selected-nav-menu" style="<?php echo do_shortcode( $nav_menu_style ); ?>">
                        <button type="button" class="button"><?php _e( 'Edit Menu','taskbot' ); ?></button>
                    </p>
                <?php endif; ?>
            </div>
            <?php
...
//register widget
function taskup_nav_menu_widgets() {
	register_widget( 'Taskup_Nav_Menu_Widget' );
}
add_action( 'widgets_init', 'taskup_nav_menu_widgets' );
